...
//		        bColor 字符背景颜色
****************************************************************************************************/
void LCD_PutChar_H(unsigned int x, unsigned int y, char c, unsigned int fColor, unsigned int bColor)
{
 unsigned int i,j;
...
//		        bColor 字符背景颜色
****************************************************************************************************/
void LCD_PutString_H(unsigned short x, unsigned short y, unsigned char *s, unsigned int fColor, unsigned int bColor) 
{
	 unsigned char l=0;                            	//显示屏位置增量
	 unsigned short i,j;
	...
	y=319-j;
     ...
     while(*s) 
	 {
		if( *s < 0x80)                             		//判断s指向的字符串中的某字符的编码值是否小于128,如果小于,即为ASCII字符
		    {
			 LCD_PutChar_H(x,y-l*8,*s,fColor,bColor);			//显示该字符
		     s++;l++;                              		//指针加1,位置加1
			}
		else
		    {
...
输入参数：x 横坐标
         ...
         *s 需要显示的字符串
         ...
         bColor 字符背景颜色
****************************************************************************************************/
void LCD_PutString24_H(unsigned short x , unsigned short y, unsigned char *s, unsigned int fColor, unsigned int bColor) {
	 unsigned char l=0;
	 unsigned short i,j;
	i=y;																											//坐标变换
	j=x;
...
void Show_RGB_H (unsigned short x0,unsigned short x1,unsigned short y0,unsigned short y1,unsigned int Color)
{
	unsigned int i,j;
